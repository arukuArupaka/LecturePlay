...
import { getOrCreatePlayerId, getSampleCells, getRandomPenalty } from '@/lib/game-utils'
import type { Room, GridSize } from '@/lib/types'
import { Grid3X3, Skull, ArrowRight, Sparkles, Grid2X2Plus } from 'lucide-react'
...
export default function BingoSetupPage() {
  ...
  const expectedCells = gridSize * gridSize
  const filledCount = cells.filter(c => c.trim()).length
    ...
    <main className="min-h-screen flex flex-col p-4 pb-8">
      <div className="w-full max-w-md mx-auto space-y-6 animate-slide-up">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-xl font-bold text-foreground">ビンゴカードを作ろう</h1>
        ...
        <Card className="bg-card border-border">
          <CardContent className="p-3">
              ...
              {cells.map((cell, index) => (
                <div key={index} className="relative">
                  <Input
                    ...
                    maxLength={20}
                    className={`
                      ${gridSize === 5 ? 'h-14 text-[10px]' : 'h-20 text-xs'}
            ...
            <p className="text-xs text-muted-foreground text-center mt-3">
              {filledCount}/{expectedCells} マス入力済み
            </p>
          </CardContent>
        </Card>
